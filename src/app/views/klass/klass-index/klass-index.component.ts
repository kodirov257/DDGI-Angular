...
@Component({
  ...
  styleUrls: ['./klass-index.component.scss']
})
export class KlassIndexComponent implements OnInit {
  ...
  displayAddForm(name: string): void {
    this.router.navigate(['/klasses/create']);
     // this.myService.openWindow(name);
  }

  displayEditForm(id: number): void {
    this.router.navigate([`/klasses/${id}/edit`]);
  }
  ...
  ngOnInit(): void {
    this.dtService.initDt('klass').toPromise()   // Get Dt options from back by code_name
      .then((response) => {
