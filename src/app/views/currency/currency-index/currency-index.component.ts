...
export class CurrencyIndexComponent implements OnInit {
  ...
  constructor(private http: HttpClient,
              ...
              private myService: CurrencyService
  ) {}

  displayAddForm(name: string): void {
    this.router.navigate(['/currencies/create']);
     // this.myService.openWindow(name);
  }

  displayEditForm(id: number): void {
    this.router.navigate([`/currencies/${id}/edit`]);
  }
  ...
  ngOnInit(): void {
    this.dtService.initDt('currency').toPromise()   // Get Dt options from back by code_name
      .then((response) => {
